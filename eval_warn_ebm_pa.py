...
def sample_p_0(device, replay_buffer, bs, y=None):
    ...
    random_samples = init_random(args, args.buffer_size)
    random_samples.data = t.Tensor(dataset.data[inds, :args.num_points]).permute(0, 2, 1)
    choose_random = (t.rand(args.buffer_size) < args.reinit_freq).float()[:, None, None]
    print(random_samples.shape, choose_random.shape)
...
def sample_q(args, device, f, replay_buffer, y=None):
    ...
    init_sample, buffer_inds = sample_p_0(device, replay_buffer, bs, y=y)
...
def cond_samples(f, replay_buffer, args, device, fresh=False):
    ...
    all_y = t.cat(all_y, 0)
    each_class = [replay_buffer[all_y == l] for l in range(5)]
...
def main(args):
    ...
    if args.ecal = "sample_one":
        sample_one(f,args,device)
